...
def FDTDataLoader(ListTypeData=[], local='', variables_dict={}):
    ...
    for TypeData in ['Validation', 'Training']:
        if TypeData in ListTypeData:
            Source = np.load(os.path.join(local, 'StepByStep', '0', 'BurstsData', 'Data', TypeData, 'PulsesAnt_0.npy'))
            Translation = np.load(os.path.join(local, 'StepByStep', '0', 'BurstsData', 'Data', TypeData, 'PDWsDCI_0.npy'))
