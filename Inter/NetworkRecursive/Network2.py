...
class MemEncoder(nn.Module):
    def __init__(self, d_in, d_att=32, n_heads=4, n_encoders=2, width_FF=[32], widths_embedding=[32], seq_len=10, n_mes=4, norm='post', dropout=0):
        super().__init__()
        self.latent_seq = LearnableParameters(torch.normal(0, 1, [1, seq_len, d_att]))
        self.pad_token = LearnableParameters(torch.normal(0, 1, [1, 1, d_att]))

        self.memory_embedding = FeedForward(d_in=d_in, d_out=d_att, widths=widths_embedding, dropout=dropout)

        self.mem_pos_encoding = PositionalEncoding(d_att=d_att, dropout=dropout, max_len=n_mes + 1)
        self.latent_pos_encoding = PositionalEncoding(d_att=d_att, dropout=dropout, max_len=seq_len + 1)

        self.memory_injector = DecoderLayer(d_att=d_att, n_heads=n_heads, norm=norm, width_FF=width_FF, dropout_A=dropout, dropout_FF=dropout)
    ...
    def forward(self, mem, pad_mask):
        mem = self.memory_embedding(mem)

        mem = (mem * (1 - pad_mask) +
               self.pad_token() * pad_mask)

        mem = self.mem_pos_encoding(mem)

        latent = self.latent_pos_encoding(self.latent_seq())
        latent = self.memory_injector(target=latent, source=mem)
...
class MemDecoder(nn.Module):
    def __init__(self, d_in, d_att=32, n_heads=4, n_encoders=2, width_FF=[32], widths_embedding=[32], seq_len=10, n_mes=4, norm='post', dropout=0):
        super().__init__()
        self.memory_seq = LearnableParameters(torch.normal(0, 1, [1, n_mes, d_att]))
        self.pad_token = LearnableParameters(torch.normal(0, 1, [1, 1, d_att]))

        self.mem_decoderFF = FeedForward(d_in=d_att, d_out=d_in, widths=list(widths_embedding.__reversed__()), dropout=0)

        self.mem_pos_encoding = PositionalEncoding(d_att=d_att, dropout=dropout, max_len=n_mes + 1)
        self.latent_pos_encoding = PositionalEncoding(d_att=d_att, dropout=dropout, max_len=seq_len + 1)

        self.latent_injector = DecoderLayer(d_att=d_att, n_heads=n_heads, norm=norm, width_FF=width_FF,
                                            dropout_A=dropout, dropout_FF=dropout)
        self.encoders = nn.ModuleList()
        for i in range(n_encoders):
            self.encoders.append(EncoderLayer(d_att=d_att, n_heads=n_heads, norm=norm, width_FF=width_FF, dropout_FF=dropout, dropout_SA=dropout))

    def forward(self, latent, pad_mask):
        mem = self.mem_pos_encoding(self.memory_seq())

        latent = self.latent_pos_encoding(latent)

        mem = self.latent_injector(target=mem, source=latent)

        for encoder in self.encoders:
            mem = encoder(mem)

        mem = (mem - self.pad_token() * pad_mask)
        mem = self.mem_decoderFF(mem)
...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, d_in, d_out, d_att=32, n_heads=4, n_encoders=3, n_decoders=3, width_FF=[32], widths_embedding=[32],
        ...
        self.mem_encoder = MemEncoder(d_in + 1, d_att=d_att, n_heads=n_heads, norm=norm, n_encoders=2, width_FF=width_FF,
                                      n_mes=n_mes, seq_len=len_in, widths_embedding=widths_embedding, dropout=dropout)
        self.mem_decoder = MemDecoder(d_in + 1, d_att=d_att, n_heads=n_heads, norm=norm, n_encoders=2, width_FF=width_FF,
                                      n_mes=n_mes, seq_len=len_in, widths_embedding=widths_embedding, dropout=dropout)

        self.input_injector = DecoderLayer(d_att=d_att, n_heads=n_heads, norm=norm, width_FF=width_FF, dropout_A=dropout, dropout_FF=dropout)
    ...
    def forward(self, source, target, mem_in, input_mask):
        ...
        latent = self.mem_encoder(mem_in, pad_mem_in_mask)

        latent = self.input_injector(target=self.enc_pos_encoding(latent), source=src)

        for encoder in self.encoders:
            latent = encoder(latent)

        mem_out = self.mem_decoder(latent, pad_mem_out_mask)
