...
class TCNet(nn.Module):
    def __init__(
        ...
        super().__init__()
        tcn_channels = [hidden_dim] * n_layers
    ...
    def step(self, input_1, input_2, next_mask=None):
        ...
        if self.buffer is None:
            self.buffer = x_t_perm
        else:
            self.buffer = torch.cat([self.buffer, x_t_perm], dim=2)
        ...
        if self.buffer.size(2) > self.max_history:
            self.buffer = self.buffer[:, :, -self.max_history:]

        # 3. Inférence TCN
        # On applique le TCN sur tout le buffer
        tcn_out_seq = self.tcn(self.buffer)  # [Batch, Channels, Buffer_Len]
