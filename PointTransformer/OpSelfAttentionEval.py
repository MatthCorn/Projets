...
class SA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        GVA2 = self.GroupedVectorAttention2(Q, K, V)

        print(torch.norm(GVA-GVA2))
        print(torch.norm(GVA))
    ...
    def GroupedVectorAttention(self, Q, K, V):
        ...
        Q_grouped = Q.view(batch_size, len_seq, n_group, d_group)
        K_grouped = K.view(batch_size, len_seq, n_group, d_group)
        V_grouped = V.view(batch_size, len_seq, d_group, n_group)

        diff_Q_K = torch.einsum('ijkl,ibkl->ijbkl', Q_grouped, -K_grouped)
        # diff_Q_K.shape = batch_size, len_seq, len_seq, n_group, d_group)

        attention = torch.mean(diff_Q_K, dim=-1)
    ...
    def GroupedVectorAttention2(self, Q, K, V):
        d_group = self.d_group
        n_group = self.n_group
