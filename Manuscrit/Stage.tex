...
Le passage d'une modélisation globale à une modélisation locale impose une adaptation de la représentation des données, notamment sur l'encodage des informations temporelles. Comme évoqué précédemment [ref chapitre antérieur], nos données brutes encodent le temps d'arrivé relativement à la position dans la séquence pour éviter la croissance incontrôlée d'un paramètre des impulsions dans des séquences de taille inconnue. Cependant, dans le contexte du dépliage où les séquences d'entrée et de sortie sont entrelacées, cette représentation est remise en question, afin de pouvoir fournir au modèle une information locale et immédiatement exploitable.
\subsubsection{Encodage relatif local du temps}
L'objectif est de fournir au réseau des différentiels de temps pertinents pour sa décision courante, tout en respectant l'esprit de l'approche qui incite le modèle a reproduire dans son espace latent, à travers sa mémoire, une information semblable à la mémoire du DCI.
\begin{itemize}
\item Pour les PDW incidents ($(I_i)_{1 \leq i \leq N}$) : \\ L'information temporelle est préservée par le processus de dépliage (car il est réversible). Cependant, pour permettre au modèle de prédire, étant donné le PDW incident $I_i$ le bon nombre de PDW à générer avant le prochain PDW incident $I_{i+1}$, nous enrichissons la représentation de chaque PDW  $I_i$. Nous concaténons à ses caractéristiques la différence de temps d'arrivé avec l'impulsion suivante : $\Delta T_{i} = TOA(I_{i+1}) - TOA(I_{i})$.
