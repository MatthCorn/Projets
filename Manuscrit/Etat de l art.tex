...
\section{Environnements Numériques et fondements de l'Intelligence Artificielle}
...
\subsection{Distinction conceptuelle : Environnement Virtuel et Jumeau Numérique}
...
\subsection{Principes mathématiques de l'Apprentissage Profond}
Avant d'explorer les architectures génératives et séquentielles avancées, il convient de formaliser les fondements mathématiques de l'apprentissage profond (\textit{Deep Learning}). Si le vocabulaire associé à l'intelligence artificielle emprunte souvent à la biologie, son mécanisme relève en réalité de l'optimisation mathématique de fonctions paramétriques.

\subsubsection{Risque théorique, risque empirique et surapprentissage}
...
C'est de cet enchevêtrement de couches que le modèle tire son qualificatif de "profond" (\textit{deep}). L'ensemble des paramètres $\theta = \{\theta_1, \dots, \theta_L\}$ (les poids et les biais de chaque couche $(f_l)_{1 \leq l \leq L}$) peut se compter en millions, voire milliards.\\

L'objectif de l'apprentissage est de trouver le jeu de paramètres optimal $\theta^*$ qui minimise le risque théorique, caractérisant la qualité de reconstruction des prédictions futures en les comparant à la vérité terrain. La qualité de reconstruction est calculé via la fonction de perte notée $\mathcal{L}(\hat{y}, y)$, qui mesure l’écart entre la prédiction $\hat{y}$ et la référence $y$. Le problème d'optimisation se formule comme la minimisation du risque théorique, c'est-à-dire l'espérance de la perte $\mathcal{L}$ sur la distribution de probabilité conjointe $P(x,y)$ régissant le phénomène physique étudié :
$$\theta^* = \arg\min_\theta \mathbb{E}_{(x,y) \sim P}[\mathcal{L}(f_\theta(x), y)]$$

Cependant, la distribution $P$ étant inconnue, cette espérance ne peut être calculée analytiquement. La stratégie consiste alors à approximer cette espérance par une moyenne calculée sur un jeu de données d'entraînement fini, constitué de $N$ exemples $(x_i, y_i)$. Le problème se transforme ainsi en la minimisation du risque empirique :
$$\hat{\theta}^* = \arg\min_\theta \frac{1}{N} \sum_{i=1}^N \mathcal{L}(f_\theta(x_i), y_i)$$

Cette substitution de l'espérance par une moyenne empirique met en lumière l'une des limites fondamentales de l'apprentissage automatique : le surapprentissage (\textit{overfitting}) : si l'espace des solutions modélisables est trop permissif, le réseau parvient à exploiter ses nombreux degrés de liberté pour mémoriser parfaitement l'échantillon d'entraînement. Le risque empirique tombe strictement à zéro, mais au prix d'oscillations extrêmes et aberrantes entre ces points, faisant diverger le risque théorique.\\
...
En pratique, on constitue un échantillon de données indépendant, tenu à l'écart de la phase d'optimisation, appelé ensemble de validation. Cet ensemble ne pourra donc pas être influencé positivement par un sur-apprentissage et permet évaluer la capacité de généralisation d'un modèle.

\subsubsection{Optimisation par descente de gradient et rétropropagation}
...
Dans le cadre de l'apprentissage profond, la succession des couches $(f_1, \dots, f_L)$ est systématiquement entrelacée de fonctions d'activation non-linéaires. Cette non-linéarité est essentielle pour modéliser des phénomènes complexes, mais elle rend le paysage d'optimisation non-convexe. Il devient indispensable de recourir à une méthode d'optimisation itérative : la descente de gradient. À chaque itération, les paramètres sont mis à jour dans la direction opposée au gradient de l'erreur, selon un pas d'apprentissage $\alpha$ :
$$\theta \leftarrow \theta - \alpha \nabla_\theta \mathcal{L}$$

Pour évaluer ce gradient sur un réseau profond, on formalise le flux d'information par une suite d'états intermédiaires. En posant $a_0 = x$ comme vecteur d'entrée, la sortie de chaque couche $l$ se définit par la récurrence $a_l = f_l(a_{l-1}; \theta_l)$. La sortie de la dernière couche correspond ainsi à la prédiction finale du modèle : $a_L = \hat{y}$. \\

Le calcul du gradient $\nabla_\theta \mathcal{L}$ pour une matrice de paramètres spécifique $\theta_l$, située dans les couches profondes, repose sur la formule de dérivation des fonctions composées (\textit{chain rule}). La dérivée s'exprime alors comme le produit des matrices Jacobiennes successives le long du graphe de calcul :

$$\frac{\partial \mathcal{L}}{\partial \theta_l} = \frac{\partial \mathcal{L}}{\partial a_L} \frac{\partial a_L}{\partial a_{L-1}} \dots \frac{\partial a_{l+1}}{\partial a_l} \frac{\partial a_l}{\partial \theta_l}$$

Son implémentation algorithmique, connue sous le nom de rétropropagation du gradient (\textit{backpropagation}), permet de calculer ces termes informatiquement et efficacement en remontant la chaîne de dérivées partielles de la sortie vers l'entrée du réseau .

\subsubsection{Géométrie de l'optimisation}
...
\subsubsection{Reconstruction neurale et représentations implicites}
...
\subsubsection{Synthèse géométrique par modèles de diffusion}
...
\subsubsection{Apprentissage sous contraintes physiques (Physics-Informed)}
...
\subsubsection{Apprentissage structuré par biais inductif physique}
...
\subsubsection{Intégration d'agents autonomes par apprentissage par renforcement} 
...
\subsubsection{Génération procédurale et apprentissage par curriculum} 
...
\section{Architectures neuronales génératives}
...
\subsection{Auto-Encodeurs Variationnels (VAE)}
...
\subsection{Réseaux Antagonistes Génératifs (GAN)}
...
\subsection{Modèles de Diffusion Probabilistes}
...
\section{Architectures de traitement séquentiel et spatial}

\subsection{Typologie des données : causalité temporelle et topologie spatiale}
...
\subsubsection{Propriétés causales des données séquentielles}
...
\subsubsection{Topologie et contiguïté des données spatiales}
...
\subsubsection{Origine et application à la vision par ordinateur}
...
\subsubsection{Filtrage local et expansion hiérarchique du champ récepteur}
...
\subsubsection{Application aux structures spatiales bidimensionnelles}
...
\subsubsection{Application aux séquences unidimensionnelles}
...
\subsubsection{Généralisation aux données volumétriques et topologies irrégulières}
...
\subsubsection{Évolution architecturale : des RNN simples aux portes logiques (LSTM/GRU)}
...
\subsubsection{Principe de récurrence et mise à jour de l'état mémoire}
...
\subsubsection{Modèles d'Espaces d'États (SSM)}
...
\subsubsection{Traitement du langage naturel et architectures Sequence-to-Sequence}
...
\subsubsection{AApplication aux systèmes dynamiques et temporels continus}
...
\subsubsection{Limites de la récurrence et introduction du mécanisme d'Attention}
...
\subsubsection{Architecture macroscopique Encodeur-Décodeur}
...
\subsubsection{Le mécanisme d'Attention et ses variantes}
...
\subsubsection{Application au traitement du langage naturel}
...
\subsubsection{Application à la vision par ordinateur (Vision Transformers)}
...
\subsubsection{Généralisation aux systèmes physiques et à la prise de décision}
