...
\subsection{Début des contres-mesures} 
...
\subsection{Apparition de l'agilité fréquentielle} 
...
\subsection{La cohérence, le Doppler et l'analyse fine} 
...
\subsection{Limites et définition de la problématique}

Bien que cet environnement virtuel offre une capacité de génération de données maîtrisée et en grande quantité, sa mécanique de modélisation s'avère particulièrement lente. Cette lenteur, qui peut transformer la simulation d'une minute de scénario tactique en plusieurs heures de calcul,  devient problématique lors de la simulation de scénarios complexes impliquant un grand nombre de radars. De surcroît, elle interdit toute exécution en temps réel, un cas d'usage pourtant indispensable pour la formation des opérateurs sur des simulateurs interactifs, où l'humain doit pouvoir modifier la situation tactique en direct, par exemple en initiant des manœuvres d'évitement.

Nos travaux de recherche s'inscrivent précisément dans ce contexte. L'objectif de cette thèse est d'identifier le goulot d'étranglement de l'environnement virtuel et d'étudier comment l'intelligence artificielle peut s'y substituer pour accélérer la génération des données en conservant la même représentativité. Comme nous le verrons, ce goulot correspond à un bloc nommé \textit{Détection et Caractérisation des Impulsions} dont le rôle est de modéliser les effets qu'ont sur le flux de PDW à la fois la détection spectrale et le suivis temporel des informations. 
