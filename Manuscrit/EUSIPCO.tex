...
\end{equation}
La tâche du modèle consiste à prédire l'ordonnancement des vecteurs $V_i$ induit par le tri croissant des scores $S_i$, en se basant uniquement sur l'observation des données, sans connaissance a priori de la règle d'ordonnancement. Ce principe est illustré dans la figure \ref{fig:sort}.

\begin{figure}[htbp]
\begin{center} 
\includegraphics[scale=0.4]{Sorting.png}
\end{center}
\caption{Diagramme de la procédure de tri décrite. Les différentes étapes sont visibles de haut en bas : départ de la séquence désordonnée, association d'un score par élément, ordonnancement par critère de score croissant et enfin restitution de la séquence ordonnée.}
\label{fig:sort}
\end{figure}
