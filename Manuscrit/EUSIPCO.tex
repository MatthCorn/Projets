\chapter{Évaluation de la viabilité par l'ordonnancement supervisé}
\label{chap:eusipco}
\section{Contexte et motivations}
Le chapitre précédent a introduit le DCIS, fruit d'une démarche de simplification visant à obtenir un environnement d'apprentissage contrôlé sur lequel obtenir nos premiers résultats. En réalité, la première tentative infructueuse d'appliquer une architecture Transformer nous a poussés à poursuivre cette quête de simplification. Nous cherchions un test de viabilité tel que l'échec de l'architecture Transformer sur cette tâche signalerait la nécessité de repenser entièrement l'approche de cette thèse.

Comme présenté dans le chapitre \ref{chap:prob}, la création d'une impulsion interceptée par le DCI repose en partie sur l'extraction des Détections Non-Ambiguës (DNA) sur chaque palier. Ce traitement exige la comparaison de caractéristiques sur de larges plages (de fréquence et d'amplitude notamment), avec des niveaux de précision variant selon le scénario. Le test de validité doit ainsi être un problème élémentaire, faisant appel à des comparaisons, et dont la précision nécessitée pour traiter un exemple peut être quantifiée. Nous avons alors choisi un problème que nous qualifions de "tri".

Ce tri peut être considéré comme une simplification ultime du traitement d'extraction des DNA, où seules les impulsions de plus hautes amplitudes seraient sélectionnées. Évaluer la capacité du modèle à trier une séquence de vecteurs permet non seulement de vérifier son aptitude à réordonner des données, mais surtout de quantifier jusqu'à quel niveau de proximité numérique le réseau parvient à déterminer "quel vecteur l'emporte".

Pour que cette évaluation puisse refléter les capacités de discernement dont aurait besoin le Transformer pour la transduction, une contrainte méthodologique s'impose. L'apprentissage de la transduction ne permettant pas d'indiquer explicitement au réseau quelles caractéristiques comparer, l'apprentissage du tri ne doit pas non plus laisser transparaître la logique à la règle. Ainsi, l'apprentissage et l'architecture doivent être totalement agnostiques du problème.

Le modèle recevra en entrée une séquence de vecteurs désordonnés évoluant dans un espace continu $R^d$ et doit prédire la séquence correspondante réordonnée selon une règle implicite, en minimisant une erreur de reconstruction. La logique de cette règle, décrite en \ref{seq:ordreg}, ne sera exploitée ni pour aider l'entraînement, ni pour architecturer le modèle, mais uniquement pour créer les données de ce problème d'apprentissage supervisé.

\textit{Note : Notre démarche expérimentale prévoyait initialement une progression en trois étapes : apprendre l'ordonnancement pur, puis la sélection de données (simulant la proximité fréquentielle), et enfin la combinaison des deux (simulant l'extraction complète des DNA). En raison des contraintes temporelles, seule la première étape (le problème de tri) a été étudiée de manière exhaustive et a fait l'objet d'une publication \cite{cornu_learning_2025}.}

\section{Positionnement scientifique}
Si le problème du tri assisté par l'intelligence artificielle suscite un intérêt croissant, la littérature s'est jusqu'à présent structurée autour de deux motivations distinctes.

La première vise l'optimisation algorithmique pure : l'objectif est d'accélérer le processus de tri en intégrant l'apprentissage automatique au sein d'algorithmes classiques \cite{kraska_case_2018, zhu_deep_2024, ferragina_balanced_2024} ou en les substituant totalement par des fonctions différentiables \cite{kim_generalized_2024}. Ces méthodes induisent par conséquent de forts biais structurels spécifiquement liés à la tâche d'ordonnancement, contrevenant à notre besoin d'évaluation agnostique.

La seconde motivation, à laquelle nos travaux se rapportent, relève de l'analyse des capacités algorithmiques fondamentales des réseaux de neurones, au cœur du domaine de l'interprétabilité mécaniste (\textit{mechanistic interpretability} \cite{olah_zoom_2020}). L'objectif ici est d'évaluer la capacité intrinsèque des architectures de traitement de séquence, et particulièrement des Transformers, à appréhender une relation d'ordre. Toutefois, ces études se sont systématiquement restreintes à des espaces d'éléments finis (des intervalles d'entiers discrets) \cite{urdshals_structure_2025, matthew_one_nodate}. Cette discrétisation garantit par définition une séparabilité minimale absolue entre les valeurs, éludant ainsi la difficulté fondamentale du discernement fin requise pour le traitement des variables radar.

À notre connaissance, notre étude \cite{cornu_learning_2025} est la première à évaluer les capacités de discernement d'architectures de traitement de séquence via l'apprentissage purement agnostique d'un problème de tri en espace continu.

Constituant la première contribution de ce manuscrit, ce chapitre reprend les résultats de nos travaux publiés à la conférence EUSIPCO \cite{cornu_learning_2025}. Nous y démontrons qu'il est possible, pour une architecture de traitement de séquence, d'apprendre une relation d'ordre dans un espace continu de manière purement agnostique. Au-delà de cette preuve de concept, nous établissons que l'optimisation d'une telle tâche exige de repenser l'approche d'apprentissage standard pour en maximiser les performances. Nous détaillons ainsi deux contributions méthodologiques majeures, issues de nos premiers échecs et conçues pour être systématiquement réinvesties dans la suite de nos modélisations sur le simulateur DCI :

\begin{itemize}
\item Une fonction de coût pondérée, équilibrant l'impact de l'amplitude de chaque séquence dans le calcul du gradient.
\item Une stratégie de fenêtrage des données permettant un apprentissage par curriculum sur la difficulté de séparabilité.
\end{itemize}
...
\section{Conclusion}
Les travaux sur ce problème de tri ont eu un impact majeur sur la thèse. Ils ont permis d'en apprendre davantage sur les capacités de discernement des modèles de traitement de séquence. On a pu établir une supériorité de l'architecture Transformer sur un réseau CNN. Nous avons aussi introduit deux méthodes, la pondération de l'erreur et l'apprentissage progressif, qui pourront être ré-exploitées dans la suite des travaux. Et surtout, nous considérons que le test de viabilité est validé. Ainsi, bien qu'on ne soit pas sûr que notre approche puisse réellement aboutir, nous sommes rassurés et considérons que le Transformer dispose des capacités de traitement nécessaires pour parvenir à réaliser une transduction entre séquence de PDW incidents et séquence de PDW interceptés.