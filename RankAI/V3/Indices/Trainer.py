from RankAI.V3.Indices.Ranker import Network, ChoseOutput
from RankAI.V3.Indices.DataMaker import MakeData
...
if save:
    ...
    save_path = os.path.join(local, 'RankAI', 'Save', 'V3', 'Indices', folder)
################################################################################################################################################

param = {'n_encoder': 1,
         'path_ini': None,
         # 'path_ini': os.path.join('RankAI', 'Save', 'V3', 'Indices', 'XXXXXXXXXX', 'ParamObs.pkl'),
         'len_in': 10,
         'd_in': 10,
         'd_att': 64,
         'WidthsEmbedding': [32],
...
for j in tqdm(range(n_iter)):
    ...
    for p in range(n_minibatch):
        ...
        for k in range(n_batch):
            ...
            err = torch.norm(Prediction-OutputBatch, p=2)/sqrt(batch_size*NInput)
            ...
            perf += float(torch.sum(ChoseOutput(Prediction, NInput) == OutputBatch))/(NDataT*NInput)
    ...
    with torch.no_grad():
        ...
        err = torch.norm(Prediction - Output, p=2)/sqrt(NDataV*NInput)
        ValidationError.append(float(err))
        ValidationPerf.append(float(torch.sum(ChoseOutput(Prediction, NInput) == Output))/(NDataV*NInput))
