from RankAI.V3.Vecteurs.DataMaker import MakeData
from RankAI.V3.Vecteurs.Ranker import Network, ChoseOutput
...
if save:
    ...
    save_path = os.path.join(local, 'RankAI', 'Save', 'V3', 'Vecteurs', folder)
################################################################################################################################################

param = {'n_encoder': 1,
         'len_in': 10,
         'path_ini': None,
         # 'path_ini': os.path.join('RankAI', 'Save', 'V3', 'Vecteurs', 'XXXXXXXXXX', 'ParamObs.pkl'),
         'd_in': 10,
         'd_att': 64,
         'WidthsEmbedding': [32],
         'n_heads': 4,
...
ValidationInput, ValidationOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataV, WeightF=WeightF, WeightN=WeightN)
...
TrainingInput, TrainingOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataT, WeightF=WeightF, WeightN=WeightN)
...
for j in tqdm(range(n_iter)):
    ...
    for p in range(n_minibatch):
        ...
        for k in range(n_batch):
            ...
            err = torch.norm(Prediction-OutputBatch, p=2)/sqrt(batch_size*DVec*NInput)
            ...
            perf += float(torch.sum(ChoseOutput(Prediction, InputBatch) == ChoseOutput(OutputBatch, InputBatch)))/(NDataT*NInput)
    ...
    with torch.no_grad():
        ...
        err = torch.norm(Prediction - Output, p=2) / sqrt(NDataV*DVec*NInput)
        ValidationError.append(float(err))
        ValidationPerf.append(float(torch.sum(ChoseOutput(Prediction, Input) == ChoseOutput(Output, Input)))/(NDataV*NInput))
