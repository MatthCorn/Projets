...
from RankAI.V0.OneHot.DataMaker import MakeData
...
param = {'n_encoder': 1,
         'path_ini': None,
         # 'path_ini': os.path.join('RankAI', 'Save', 'V0', 'OneHot', 'XXXXXXXXXX', 'ParamObs.pkl'),
         'len_in': 10,
         'd_in': 10,
         'd_att': 64,
         'WidthsEmbedding': [32],
...
if True:
    ...
    if param['type_error'] == 'CE':
        ax1.plot([float(torch.log(torch.tensor(NVec)))] * len(ValidationError), 'black')
    elif param['type_error'] == 'MSE':
