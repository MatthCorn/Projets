...
# SIMPLE COSINE LR SCHEDULER :
################################################################################################################################################
'''
Gradually reduces the learning rate from a maximum value (max_lr) to a small minimum, following a cosine curve.
'''
class CosineScheduler(LambdaLR):
    def __init__(self, optimizer, warmup_steps, max_steps, max_lr, base_lr=1e-7, last_epoch=-1, verbose=False):
