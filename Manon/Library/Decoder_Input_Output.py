...
def build_decoder_input_output_parallel(TrainingInput, TrainingOutput, TrainingMasks):
    ...
    ################################################################################################################################################
    valid_output_len = TrainingMasks[1].sum(dim=1).long().squeeze(1) - 1  # [B] => correspond to mult_mask sum for each sequence in batch
    ...
    arr_A = torch.cat([padded_input, padded_end_token_vec, TrainingOutput], dim=1)  # [B, T, d_out] w/ T = len_in (input) + 1 (end token vec) + len_out (output)
    ...
    decoder_input_with_dt = torch.cat([current_inputs, decoder_output_tensor, delta_norm.unsqueeze(-1)], dim=-1) # [B, T, 2*d_out + 1] """
