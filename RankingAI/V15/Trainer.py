from RankingAI.V15.Ranker import Network
from RankingAI.V15.DataMaker import MakeData
...
if save:
    ...
    save_path = os.path.join(local, 'RankingAI', 'Save', 'V15', folder)
    os.mkdir(save_path)
################################################################################################################################################

param = {'n_encoder': 2,
         'path_ini': None,
         # 'path_ini': os.path.join('RankingAI', 'Save', 'V15', '2024-07-15__09-39', 'ParamObs.pkl'),
         'len_in': 10,
         'd_in': 10,
         'd_att': 64,
         'WidthsEmbedding': [32],
...
N = Network(n_encoder=param['n_encoder'], len_in=param['len_in'], len_latent=param['LenOut'], d_in=param['d_in'], d_att=param['d_att'],
...
ValidationInput, ValidationOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataV,
                                             WeightsF=WeightsF, WeightsN=WeightsN, NOutput=NOutput)
...
TrainingInput, TrainingOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataT,
                                         WeightsF=WeightsF, WeightsN=WeightsN, NOutput=NOutput)
...
for j in tqdm(range(n_iter)):
    ...
    with torch.no_grad():
        ...
        ValidationError.append(float(err)/sqrt(NDataV*NOutput*DVec))

    TrainingError.append(error/sqrt(batch_size*NOutput*DVec))
