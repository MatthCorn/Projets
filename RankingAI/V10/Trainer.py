from RankingAI.V10.Ranker import Network
from RankingAI.V10.DataMaker import MakeData
...
if save:
    ...
    save_path = os.path.join(local, 'RankingAI', 'Save', 'V10', folder)
    os.mkdir(save_path)
################################################################################################################################################
...
N = Network(n_encoder=param['n_encoder'], len_in=param['len_in'], len_latent=param['len_latent'], d_in=param['d_in'],
...
ValidationInput, ValidationOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataV, WeightsCut=WeightsCut, WeightsSort=WeightsSort, LenOut=LenOut)
...
TrainingInput, TrainingOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataT, WeightsCut=WeightsCut, WeightsSort=WeightsSort, LenOut=LenOut)
...
for j in tqdm(range(n_iter)):
    ...
    with torch.no_grad():
        ...
        ValidationError.append(float(err)/sqrt(NDataV*DVec*LenOut))

    TrainingError.append(error/sqrt(batch_size*DVec*LenOut))
