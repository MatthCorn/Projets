from RankingAI.V9.Ranker import Network
from RankingAI.V9.DataMaker import MakeData
...
if save:
    ...
    save_path = os.path.join(local, 'RankingAI', 'Save', 'V9', folder)
    os.mkdir(save_path)
################################################################################################################################################
...
N = Network(n_encoder=param['n_encoder'], len_in=param['len_in'], d_in=param['d_in'], d_att=param['d_att'], WidthsEmbedding=param['WidthsEmbedding'],
            n_heads=param['n_heads'], norm=param['norm'])
...
ValidationInput, ValidationOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataV, WeightsCut=WeightsCut, WeightsSort=WeightsSort)
...
warmup_steps = int(NDataT / batch_size) * 5
# lr_scheduler = None
lr_scheduler = Scheduler(optimizer, 256, warmup_steps, max=param['max_lr'])

TrainingInput, TrainingOutput = MakeData(NInput=NInput, DVec=DVec, sigma=1, NData=NDataT, WeightsCut=WeightsCut, WeightsSort=WeightsSort)
