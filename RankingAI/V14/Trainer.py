from RankingAI.V14.Ranker import Network
from RankingAI.V14.DataMaker import MakeData
...
if save:
    ...
    save_path = os.path.join(local, 'RankingAI', 'Save', 'V14', folder)
    os.mkdir(save_path)
################################################################################################################################################
...
for j in tqdm(range(n_iter)):
    ...
    for p in range(n_minibatch):
        ...
        for k in range(n_batch):
            ...
            Prediction = N(InputBatch, OutputBatch)
    ...
    with torch.no_grad():
        ...
        Prediction = N(Input, Output)

        err = torch.norm(Prediction - Output, p=2)
        ValidationError.append(float(err)/sqrt(NDataV*NInput*DVec))

    TrainingError.append(error/sqrt(batch_size*NInput*DVec))
