...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, d_pulse, d_PDW, d_input_encoder=10, d_input_decoder=10, d_att=32, num_heads=4, num_encoders=3, n_tracker=4,
                 num_decoders=3, NbPDWsMemory=10, target_len=80, num_flags=4, device=torch.device('cpu'), FPIC=False):
        super().__init__()
        self.device = device
        self.d_PDW = d_PDW
        ...
        for i in range(num_encoders):
            self.Encoders.append(EncoderLayer(d_model=d_input_encoder, d_att=d_att, num_heads=num_heads, relative=False, MHADropout=0, FFDropout=0))

        self.Decoders = nn.ModuleList()
        Decoder = DecoderLayer(d_model=d_input_decoder, d_att=d_att, num_heads=num_heads, relative=False, MHADropout=0, FFDropout=0)
        ...
        self.SourceEmbedding = FeedForward(d_in=d_pulse, d_out=d_input_encoder, widths=[16], dropout=0)
        self.TargetEmbedding = FeedForward(d_in=d_PDW+num_flags, d_out=d_input_decoder, widths=[16], dropout=0)

        self.PhysicalPrediction = FeedForward(d_in=d_input_decoder, d_out=d_PDW, widths=[16, 32, 16], dropout=0)
        self.FlagsPrediction = FeedForward(d_in=d_input_decoder, d_out=num_flags, widths=[32, 8], dropout=0)

        # Ce vecteur a pour but de déterminer l'action a réaliser, mettre fin à la traduction dans notre cas particulier
        self.ActionPrediction = FeedForward(d_in=d_input_decoder, d_out=1, widths=[32, 8], dropout=0)

        if self.FPIC:
            # Il faut donner à ce classifier l'impulsion fictive transportée dans l'espace des PDWs
            self.PulseToPDWs = FeedForward(d_in=d_input_encoder, d_out=d_input_decoder, widths=[], dropout=0)

        self.to(device)

        self.PEEncoder = PositionalEncoding(d_model=d_input_encoder, dropout=0, max_len=target_len, device=device)
        self.PEDecoder = PositionalEncoding(d_model=d_input_decoder, dropout=0, max_len=target_len, device=device)
