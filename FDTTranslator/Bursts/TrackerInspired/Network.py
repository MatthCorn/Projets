...
def MakePermutMask(n_trackers, target_len):
    Permut = (torch.tril(torch.ones(n_trackers * target_len, n_trackers * target_len), n_trackers) -
              torch.tril(torch.ones(n_trackers * target_len, n_trackers * target_len), n_trackers - 1) +
              torch.tril(torch.ones(n_trackers * target_len, n_trackers * target_len), -n_trackers * (target_len - 1)) -
              torch.tril(torch.ones(n_trackers * target_len, n_trackers * target_len), -n_trackers * (target_len - 1) - 1))
...
def MakeTrackerMask(n_trackers, target_len):
    mask = torch.zeros(n_trackers*target_len, n_trackers*target_len)
...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, d_pulse, d_PDW, d_latent=32, num_heads=4, num_encoders=3, n_trackers=4,
        ...
        Decoder = DecoderLayer(d_model=d_latent, d_att=d_latent, num_heads=num_heads, target_len=target_len*n_trackers, relative=False, MHADropout=0, FFDropout=0)
        Decoder.SelfAttentionLayer.mask = MakePermutMask(n_trackers, target_len)
        self.Decoders.append(Decoder)
        for i in range(num_decoders-1):
            Decoder = DecoderLayer(d_model=d_latent, d_att=d_latent, num_heads=num_heads, target_len=target_len*n_trackers, relative=False, MHADropout=0, FFDropout=0)
            Decoder.SelfAttentionLayer.mask = MakeTrackerMask(n_trackers, target_len)
            self.Decoders.append(Decoder)
        ...
        self.TrackerEmbedding = TrackerEmbeddingLayer(n_trackers=n_trackers, d_input_decoder=d_latent, d_att=d_latent)
    ...
    def forward(self, source, target):
        ...
        # trg.shape = (batch_size, n_trackers*target_len, d_latent)
        ...
        trg = trg.reshape(batch_size, -1, self.n_trackers*self.d_latent)[:, :, :self.d_latent]
        # trg.shape = (batch_size, target_len, d_latent)
