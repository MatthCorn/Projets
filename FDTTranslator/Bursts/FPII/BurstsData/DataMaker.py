...
def WriteBatchs(Source, Translation, BatchSize, TypeData):
    try:
        shutil.rmtree(os.path.join(local, 'FDTTranslator', 'Bursts', 'FPII', 'BurstsData', 'Data', TypeData))
    except:
        None

    os.mkdir(os.path.join(local, 'FDTTranslator', 'Bursts', 'FPII', 'BurstsData', 'Data', TypeData))
    ...
    for i in range(NbBatch):
        ...
        SourceFName = os.path.join(local, 'FDTTranslator', 'Bursts', 'FPII', 'BurstsData', 'Data', TypeData, 'PulsesAnt_{}.npy'.format(i))
        TranslationFName = os.path.join(local, 'FDTTranslator', 'Bursts', 'FPII', 'BurstsData', 'Data', TypeData, 'PDWsDCI_{}.npy'.format(i))
